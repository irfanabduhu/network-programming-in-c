...
int main()
{
	...
	hints.ai_flags =
		AI_PASSIVE; // bind the wildcard address; so we can listen
	...
	if (bind(socket_listen, bind_address->ai_addr,
		...
		fprintf(stderr,
			"could not bind an address to listener socket.(%d)\n",
			errno);
		return 1;
	}
	...
	// [sockaddr_storage client_address].
	int socket_client =
		accept(socket_listen, (sockaddr *)&client_address, &client_len);
	...
	getnameinfo(
		(sockaddr *)&client_address, client_len, address_buffer,
